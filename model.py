...
class ProposalLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        proposals = utils.batch_slice([boxes, scores], nms,
                                      self.config.IMAGES_PER_GPU)
        proposals = tf.slice(proposals, [0, 0, 0], [-1, self.proposal_count, -1])
        return proposals
...
class PyramidROIAlign(KL.Layer):
    ...
    def call(self, inputs):
        ...
        shapes = inputs[0].shape[:2] + self.pool_shape + (inputs[2].shape[-1], )
        shapes = shapes.as_list()
        # shapes[0] = -1
        pooled = tf.slice(pooled, [0, 0, 0, 0, 0], shapes)
        return pooled
...
class DetectionTargetLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        outputs[0] = tf.slice(outputs[0], [0, 0, 0], [-1, self.config.TRAIN_ROIS_PER_IMAGE, -1])
        outputs[2] = tf.slice(outputs[2], [0, 0, 0], [-1, self.config.TRAIN_ROIS_PER_IMAGE, -1])
        outputs[1] = tf.slice(outputs[1], [0, 0], [-1, self.config.TRAIN_ROIS_PER_IMAGE])
        outputs[3] = tf.slice(outputs[3], [0, 0, 0, 0], [-1, self.config.TRAIN_ROIS_PER_IMAGE, -1, -1])
        return outputs
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    target_masks = KB.reshape(target_masks, (-1, mask_shape[2], mask_shape[3]))
    pred_shape = tf.shape(pred_masks)
    pred_masks = KB.reshape(pred_masks,
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            gt_boxes = KL.Lambda(lambda x: norm_boxes_graph(
                x, KB.shape(input_image)[1:3]))(input_gt_boxes)
        ...
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                ...
                target_rois = KL.Lambda(lambda x: norm_boxes_graph(
                    x, KB.shape(input_image)[1:3]))(input_rois)
            else:
    ...
    def compile(self, learning_rate, momentum):
        ...
        reg_losses = [
            K.regularizers.l2(self.config.WEIGHT_DECAY)(w) / tf.cast(tf.size(w), tf.float32)
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            ...
            K.callbacks.ModelCheckpoint(self.checkpoint_path,
